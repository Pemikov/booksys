    ...
    try {
        ...
        if (!customer_name || !customer_email || !booking_date || !start_time || !end_time) {
            return res.status(400).json({ 
                success: false, 
                error: 'Missing required fields: name, email, booking_date, start_time, end_time' 
            });
        }
    ...
    try {
        ...
        const result = await db.query(
            `SELECT id, booking_date, start_time, end_time, status, notes
             FROM bookings
             WHERE customer_email = $1 AND organization_id = $2
...
// Legacy support for old method names (if needed)
exports.getAvailability = exports.getAvailableSlots;
exports.bookSlot = exports.createBooking;